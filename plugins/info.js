...
alpha(
	...
	async (message, match) => {
		...
		const result = await getJson(`${config.BASE_URL}api/info/age?dob=${match}&apikey=${config.ALPHA_KEY}`);
		if (!result.status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar ALPHA_KEY: your apikey`);
		const {
			age,
			months,
			days,
...
alpha(
	...
	async (message, match) => {
		...
		const {
			result,
			status
		} = await getJson(`${config.BASE_URL}api/info/country?code=${match}&apikey=${config.ALPHA_KEY}`);
		if (!status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar ALPHA_KEY: your apikey`);
		const {
			id,
			name,
			language,
