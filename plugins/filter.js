...
alpha({
    ...
    fromMe : true
        ...
        if(!match.includes('=')) return await message.send('_*Example:* filter alpha=https://img.png type/sticker_');
            let [pattern, msg] = match.split('=');
            let type = 'text';
            if(!msg) return await message.send('_*Example:* filter alpha=https://img.png type/image\n_*values:* text, sticker, audio, image, vedio_');
