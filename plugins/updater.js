...
alpha({
	...
	if (commits.total === 0) {
		...
		let alphaupdate = lang.HEROKU.LIST_UPDATE;
		commits['all'].map(
			(commit) => {
				alphaupdate += "```" + lang.HEROKU.COMMITS.format(commit.date.substring(0, 10), commit.message, commit.author_name) + "```\n\n";
			});
		return await message.send(alphaupdate);
	}
}
});
