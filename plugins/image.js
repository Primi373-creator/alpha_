...
alpha({
    ...
    const data = await getJson(config.BASE_URL+'api/search/gis?text='+text+`&count=${number}&apikey=${config.ALPHA_KEY}`);
    const {result} = data;
    if(!data.status) return await message.send(`API key limit exceeded. Get a new API key at ${config.BASE_URL}api/signup. Set var alpha_key: your_api_key`);
