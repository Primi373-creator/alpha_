...
alpha({
	...
	async (message, match) => {
		...
		const {
			result,
			status
		} = await getJson(`${config.BASE_URL}api/download/pinterest?url=${match}&apikey=${config.ALPHA_KEY}`);
...
alpha({
	...
	async (message, match) => {
		...
		const {
			result,
			status
		} = await getJson(`${config.BASE_URL}api/search/pinterest?text=${match}&apikey=${config.ALPHA_KEY}`);
		if (!status) return await message.send(`API key limit exceeded. Get a new API key at ${config.BASE_URL}api/signup. Set var ALPHA_KEY: your_api_key`);
		let options = [],
			n = 1;
