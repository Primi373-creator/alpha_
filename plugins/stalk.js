...
async (message, match) => {
    ...
    const { status, result } = await getJson(
        `${config.BASE_URL}api/stalk/ig?name=${encodeURIComponent(match)}&apikey=${config.ALPHA_KEY}`
    );

    if (!result || !status) return await message.send(`Please enter a new API key, as the given API key limit has been exceeded. Visit ${config.BASE_URL}api/signup to get a new API key. setvar ALPHA_KEY: your API key`);
...
alpha({
	...
	async (message, match) => {
		...
		const {
			...
			`${config.BASE_URL}api/stalk/ytchannel?name=${match}&apikey=${config.ALPHA_KEY}`
		)
...
alpha({
	...
	async (message, match) => {
		match = match || message.reply_message.text;
		if (!match)
			return await message.send('*Give me a git user name*\n*Example:*.git alpha-official')
		const {
			result,
			status
		} = await getJson(
			`${config.BASE_URL}api/stalk/github?user=${match}&apikey=${config.ALPHA_KEY}`
		)
		if (!status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar ALPHA_KEY: your apikey`);
		const {
			login,
