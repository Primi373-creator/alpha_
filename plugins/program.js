...
alpha({
    ...
    const data = await getJson(config.BASE_URL + `api/program/morece_encode?text=${match}&apikey=${config.ALPHA_KEY}`);
    if (!data.status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar ALPHA_KEY: your apikey`);
...
alpha({
    ...
    const data = await getJson(config.BASE_URL + `api/program/morece_decode?text=${match}&apikey=${config.ALPHA_KEY}`);
    if (!data.status) return await message.send(`Please enter a new apikey, as the given apikey limit has been exceeded. Visit ${config.BASE_URL}api/signup for gettig a new apikey. setvar ALPHA_KEY: your apikey`);
    return await message.send(data.result, {
        quoted: message.data
    }, 'text');
