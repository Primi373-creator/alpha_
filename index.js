...
┗•───────────────❏`
		...
		cron.schedule('*/30 * * * *', async () => {
			...
			try {
				...
				if (commit_key != Commit_key && Commit_key != "alpha") {
					await personalDB(['owner_updt'], {
						content: Commit_key
					}, 'set');
...
app.listen(config.PORT, () => console.log(`alpha Server listening on port http://localhost:${config.PORT}`));
