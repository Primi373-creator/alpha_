...
let lang;
    try {
        lang = require("./lang/"+config.LANG.toLowerCase()+".json");
        } catch (e) {
...
async function weather(message) {
	...
	try {
		...
		if (response.status === 200) return await message.client.sendMessage(message.from, {
			text: 'climet of ' + text + ' is \n\n' +
		...
		return await message.send("_*no data found on this location*_");
	}
}
...
module.exports = {
	...
	PREFIX: (!config.PREFIX || config.PREFIX == 'false' || config.PREFIX == 'null') ? "" : (config.PREFIX.includes('[') && config.PREFIX.includes(']')) ? config.PREFIX[2] : config.PREFIX.trim(),
	mode: (!config.WORKTYPE || config.WORKTYPE.toLowerCase().trim() != 'public') ? true : false,
