...
async function AudioMetaData(audio, info = {}) {
    let title = info.title || "ɪɴʀʟ-ʙᴏᴛ-ᴍᴅ";
...
async function broadcast(msg, opt, type = "group") {
    ...
    if (opt) {
        try {
            ...
            throw new Error("Inavlid JSON object");
        }
    } else opt = {};
    if (type == "group") {
        const {
            participants
        } = await msg.client.groupMetadata(msg.jid).catch(e => []);
        ...
        groupJid.map(async (jid) => {
            ...
            const {
                participants
            } = await msg.client.groupMetadata(jid).catch(e => []);
            const ujid = participants.map(a => a.id);
            for (let = 0; i < ujid.length; i++) {
                await sleep(1500);
                await msg.forwardMessage(ujid[i], msg.reply_message, opt);
            }
        });
